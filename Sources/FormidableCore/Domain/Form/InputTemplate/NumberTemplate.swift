...
extension Double: AbstractValue {
}

public struct NumberTemplate: InputTemplate {
    ...
    public init(header: Header, value: Value) {
        self.init(header: header, config: InputTemplateConfig())
        self.value = value
    }

    public mutating func actionate(inputHash: FocusableId?) -> FocusableId? {
        if inputHash == inputField {
            return nil
        }
        return inputHash
    }
