...
public extension KeyboardShortcuts.Key {
    // MARK: Letters
    ...
    var icon: String {
        if self == .`return` { return "arrow.turn.down.left" } else
        if self == .escape { return "escape" } else
        if self == .tab { return "arrow.right.to.line" } else
        if self == .space { return "space" } else
        if self == .leftArrow || self == .rightArrow { return "arrow.left.arrow.right" } else
        if self == .upArrow || self == .downArrow { return "arrow.up.arrow.down" }
        return ""
    }
}
#endif
